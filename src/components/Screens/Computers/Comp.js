...
export default function Comp() {
    ...
    const filterFunction = (data) => {
        ...
        const idstoshow = ['C3', 'C4', 'C6', 'C7'];
        const showAll = ['C1', 'C2', 'C3', 'C4', 'C5', 'C6', 'C7', 'C8'];
      
        const filterAll = (post) => idstoshow.includes(post.id) || showAll.includes(post.id);
        const filterSale = (post) => ['C3', 'C4', 'C6', 'C7'].includes(post.id);
        const filterBestSeller = (post) => ['C1', 'C2', 'C5', 'C8'].includes(post.id);
    ...
    const sortMbldata = (data, option) => {
        const discpriceIds = ['C3', 'C4', 'C6', 'C7']; // IDs of items with discprice
        const regularIds = ['C1', 'C2', 'C5', 'C8']; // IDs of regular items
    ...
    const filteredAndSortedData = sortMbldata(filterFunction(compdata), sortOption);

  return (
    <>
     <section className='mbl-sect'>
                <div className="container-fluid py-sm-5">
                    <h1 className='mbl-head text-center pt-sm-2'>Computers</h1>
                    <div className="row pt-sm-5 mt-sm-4 mt-5 ps-sm-2 justify-content-sm-between justify-content-center">
                        ...
                        <div className="col-2 d-sm-none border border-1 py-sm-2 me-sm-1 custom-select-container">
                            <select name="sort by" id="Sort" className="custom-select">
                                <option value="#"></option>
                                <option value="newest">Newest</option>
                                        <option value="priceLowToHigh">Price (Low to High)</option>
                                        <option value="priceHighToLow">Price (High to Low)</option>
