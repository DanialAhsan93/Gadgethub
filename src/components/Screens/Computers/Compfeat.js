...
export default function Compfeat() {
    const settings = {
        dots: false,
        infinite: true,
        speed: 500,
    ...
    const [slider, setslider] = useState(compdata);
    // console.log(slider)

    const location = useLocation()
    const navigate = useNavigate();
    const gotomobilefeat = (post) => {
        navigate('/compfeat', { state: { post } });
        window.scrollTo(0, 0)
    }
    ...
    // Navigate to the next product's detail page
    const goToNextProduct = () => {
        const nextProductIndex = (currentProductIndex + 1) % data.length;
        navigate(`/compfeat`, { state: { post: data[nextProductIndex] } });
    };

    // Navigate to the previous product's detail page
    const goToPrevProduct = () => {
        const prevProductIndex = (currentProductIndex - 1 + data.length) % data.length;
        navigate(`/compfeat`, { state: { post: data[prevProductIndex] } });
    };

    // const goToPrevProduct = () => {
    //         const prevProductIndex = (currentProductIndex - 1 ) % data.length;
    //         navigate(`/mblfeat`, { state: { post: data[prevProductIndex] } });
    //     }
    ...
    const saleTag = ['C4', 'C3', 'C7', 'C6'].includes(post.id);
    const remainTag = ['C1', 'C2', 'C5', 'C8'].includes(post.id);
                ...
                <section className='mobilefeat-sec'>
                    <div className="container-fluid">
                        <div className="row justify-content-around px-xxl-0 px-xl-0 px-lg-3 px-md-4 py-sm-5 mt-sm-0 mt-4 ">
                            ...
                            ms-md-0 ms-sm-0">
                                <div className="d-sm-flex d-none  justify-content-start">
                                    ...
                                    <Link style={{ textDecoration: 'none', color: 'black' }} to={'/comp'}>
                                ...
                                <div className='d-flex align-items-center d-sm-none ' style={{ fontSize: '18px' }}>
                                    <Link to={'/comp'} style={{ textDecoration: 'none', color: 'black' }}>
                                        <span><IoIosArrowBack /></span>
                                        <span>Back to Computers</span>
                                    </Link>
                                </div>
                            </div>
                        ...
                        <div className="row justify-content-center mt-sm-0 mt-5 px-lg-5 px-md-0  px-sm-5">
                            {/* padding and img classes and col-11 to col */}
                            ...
                            <div className="row pt-sm-5 mt-5 px-sm-5">
                                ...
                                <Slider {...settings}>
                                    ...
                                    {slider.map((post, id) => {
                                        ...
                                        const saleItemTag = ['C4', 'C3', 'C7', 'C6'].includes(itemid);
                                        const remainItemTag = ['C1', 'C2', 'C5', 'C8'].includes(itemid);
                                        return (
