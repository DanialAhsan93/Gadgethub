...
export default function Speaker() {
  const [data, setdata] = useState(speakdata);
    // console.log(data)
    ...
    const filterFunction = (data) => {
        ...
        const idstoshow = ['S4', 'S3', 'S7', 'S6'];
        const showAll = ['S1', 'S2', 'S5', 'S8','S4', 'S3', 'S7', 'S6'];
      
        const filterAll = (post) => idstoshow.includes(post.id) || showAll.includes(post.id);
        const filterSale = (post) => ['S4', 'S3', 'S7', 'S6'].includes(post.id);
        const filterBestSeller = (post) => ['S1', 'S2', 'S5', 'S8'].includes(post.id);
    ...
    const sortMbldata = (data, option) => {
        const discpriceIds = ['S4', 'S3', 'S7', 'S6']; // IDs of items with discprice
        const regularIds = ['S1', 'S2', 'S5', 'S8']; // IDs of regular items
    ...
    const filteredAndSortedData = sortMbldata(filterFunction(speakdata), sortOption);
  return (
    <>
    <section className='mbl-sect'>
                <div className="container-fluid py-sm-5">
                    <h1 className='mbl-head text-center pt-sm-2'>Speakers</h1>
                            ...
                            <div className="row px-sm-3 justify-content-sm-start justify-content-center">
                                {filteredAndSortedData.map((post, id) => {
                                    ...
                                    return (
                                        ...
                                        my-sm-0 mx-sm-0 my-1 mx-1 border border-1 
                                        overflow-hidden " key={id} onClick={() => gotomobilefeat(post)}>
