...
export default function Headphone() {
  const [data, setdata] = useState(headdata);
    ...
    const filterFunction = (data) => {
        ...
        const idstoshow = ['head2', 'head3', 'head5', 'head7'];
        const showAll = ['head1','head2','head3','head4','head5','head6','head7','head8'];
      
        const filterAll = (post) => idstoshow.includes(post.id) || showAll.includes(post.id);
        const filterSale = (post) => ['head2', 'head3', 'head5', 'head7'].includes(post.id);
        const filterBestSeller = (post) => ['head1', 'head2', 'head5', 'head8'].includes(post.id);
    ...
    const sortMbldata = (data, option) => {
        const discpriceIds = ['head4', 'head3', 'head7', 'head6']; // IDs of items with discprice
        const regularIds = ['head1', 'head2', 'head5', 'head8']; // IDs of regular items
    ...
    const filteredAndSortedData = sortMbldata(filterFunction(headdata), sortOption);
  return (
    <>
     <section className='mbl-sect'>
                <div className="container-fluid py-sm-5">
                    <h1 className='mbl-head text-center pt-sm-2'>Headphones</h1>
                    <div className="row pt-sm-5 mt-sm-4 mt-5 ps-sm-2 justify-content-sm-between justify-content-center">
                        ...
                        <div className="col-sm-2 d-sm-flex d-none border border-1 py-sm-2 me-sm-1" style={{width:'10.5rem'}}>
