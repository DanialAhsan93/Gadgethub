...
import './mobilefeat.css';
import { Link, useLocation, useNavigate } from 'react-router-dom';
import { mbldata } from './Mobiledata';
import { useDispatch, useSelector } from 'react-redux';
import { ADD, Manage_Toggle } from '../../../redux/action/Action';
...
export default function Mobilefeat() {
    ...
    const goToNextProduct = () => {
        ...
        navigate(`/mblfeat`, { state: { post: data[nextProductIndex] } });
    };
    ...
    const saleTag = [4, 3, 7, 6].includes(id);
    const remainTag = [1, 2, 5, 8].includes(id);

    // const isFavorite = useSelector(state => state.favoritiesReducer.isFavorite);
    // const memoizedIsFavorite = useMemo(() => isFavorite, [isFavorite]);
    const isFavorite = useSelector(state => state.favoritiesReducer.isFavorite);
    ...
    //     if (isFavorite) {
    //         console.log('Dispatching EXT action');
    //         dispatch(EXT(postId));
    //     } else {
    ...
    const handleToggleFavorite = (postId) => {
        const isCurrentlyFavorite = isFavorite[postId] || false;
            ...
            <div className="container">
                ...
                <section className='mobilefeat-sec'>
                    <div className="container-fluid px-md-2 px-0">
                        <div className="row justify-content-around px-xxl-0 px-xl-0 px-lg-3 px-md-4 py-sm-5
                         mt-sm-0 mt-4  ">
                        ...
                        <div className="row justify-content-center mt-sm-0 mt-5 px-lg-5 px-md-0 px-sm-0">
                            <div className="col-md-6 col-sm-6 padding-start-lg-35 pe-2">
                                <div className='col text-center border border-1 overflow-hidden  '>
                                    <img src={location?.state?.post.thumbnail} alt="#" className='mbl-featimg 
                                    img-fluid' />
                            ...
                            <div className="col-md-6 col-sm-6 text-start pe-lg-5 ps-sm-4">
                                ...
                                <h2 className='mblpost-title'>{location?.state?.post.title}</h2>
                             ...
                             align-items-center mt-3">
                                    ...
                                    <div className='d-sm-flex '>
                                        ...
                                        {/* {isFavorite ? (
                                            <div onClick={() => handleToggleFavorite(post.id)} style={{ cursor: "pointer" }}>
                                                <span className='ms-2 hrt-icon'>
                                                    <BsSuitHeartFill style={{ cursor: "pointer" }} />
                                                </span>
                                            </div>
                                        ) : (
                                            <div onClick={() => handleToggleFavorite(post)}>
                                                <span className='ms-2 hrt-icon'>
                        ...
                        <div className="container">
                            <div className="row pt-sm-5 mt-5 px-sm-5">
                                ...
                                <Slider {...settings}>
                                    ...
                                    {slider.map((post, id) => {
                                        const itemid = parseInt(post.id);

                                        const saleItemTag = [4, 3, 7, 6].includes(itemid);
                                        const remainItemTag = [1, 2, 5, 8].includes(itemid);
                                        return (
