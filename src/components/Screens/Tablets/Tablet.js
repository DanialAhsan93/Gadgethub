...
export default function Tablet() {
    ...
    const filterFunction = (data) => {
        ...
        const idstoshow = ['T4', 'T3', 'T7', 'T6'];
        const showAll = ['T1', 'T2', 'T5', 'T8', 'T4', 'T3', 'T7', 'T6'];

        const filterAll = (post) => idstoshow.includes(post.id) || showAll.includes(post.id);
        const filterSale = (post) => ['T4', 'T3', 'T7', 'T6'].includes(post.id);
        const filterBestSeller = (post) => ['T1', 'T2', 'T5', 'T8'].includes(post.id);
    ...
    const sortMbldata = (data, option) => {
        const discpriceIds = ['T4', 'T3', 'T7', 'T6']; // IDs of items with discprice
        const regularIds = ['T1', 'T2', 'T5', 'T8']; // IDs of regular items
    ...
    const filteredAndSortedData = sortMbldata(filterFunction(tabdata), sortOption);
    return (
        <>
            <section className='mbl-sect'>
                <div className="container-fluid py-sm-5">
                    <h1 className='mbl-head text-center pt-sm-2'>Tablets</h1>
